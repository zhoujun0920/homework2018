...
void MatrixInput::operator<<(const char* c)
{
   REPORT
   Tracer et("MatrixInput");
   if (n!=0) { n = 0; Throw(ProgramException("List of values too short - code", c)); }
}

MatrixInput::~MatrixInput()
{
   REPORT
   if (n!=0) Exit("MatrixInput: list of values too short");
}
